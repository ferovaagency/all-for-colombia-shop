...
function ShopPage() {
  ...
  useEffect(() => {
    const max = Math.max(
      ...products.map((product: any) => Number(product.sale_price ?? product.price ?? 0)),
      1000000,
    );
    setMaxPrice(max);
    setPriceRange([0, max]);
  ...
  const filtered = useMemo(() => {
    ...
    if (search.q) {
      const q = search.q;
      list = list.filter((p) =>
        fuzzyIncludes(`${p.name ?? ""} ${p.sku ?? ""} ${p.brands?.name ?? ""}`, q),
      );
    }
    if (search.categoria) {
    ...
    if (search.oferta === "1")
      list = list.filter((p) => p.sale_price && p.price && p.sale_price < p.price);
    list = list.filter((p) => {
  ...
  useEffect(() => {
    setVisibleCount(30);
  }, [search.q, search.categoria, search.marca, search.oferta, search.orden, priceRange]);
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-4">
          ...
          <Select
            ...
            onValueChange={(v) => updateSearch({ orden: v === "recientes" ? undefined : v })}
          >
        ...
        <div className="relative">
          <div
            ref={filterBarRef}
            className="flex items-center gap-2 overflow-x-auto pb-1 pr-8 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          >
            ...
            {parentCats.map((cat: any) => (
              <Chip
                key={cat.id}
                active={search.categoria === cat.slug}
                onClick={() => updateSearch({ categoria: cat.slug })}
              >
                {cat.name}
              </Chip>
            ))}
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Chip active={!!search.marca}>
                    Marca
                    {search.marca
                      ? `: ${brands.find((b) => b.slug === search.marca)?.name ?? ""}`
                      : ""}
            ...
            <Chip
              active={search.oferta === "1"}
              onClick={() => updateSearch({ oferta: search.oferta === "1" ? undefined : "1" })}
            >
