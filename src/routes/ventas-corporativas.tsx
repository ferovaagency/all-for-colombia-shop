      ...
      <section className="bg-gradient-hero text-primary-foreground py-20">
        <div className="container mx-auto px-4">
          ...
          <p className="text-lg text-white/85 max-w-2xl">
            Equipos tecnológicos, aires acondicionados, plóters y más para tu empresa. Precios
            especiales en compras por volumen.
          </p>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          <Benefit
            ...
            desc="Descuentos especiales según la cantidad"
          />
          <Benefit
            icon={<FileCheck className="h-6 w-6" />}
            title="Facturación a empresa"
            desc="Documentos en regla para tu contabilidad"
          />
          <Benefit
            icon={<Headphones className="h-6 w-6" />}
            title="Soporte dedicado"
            desc="Asesor exclusivo post-venta"
          />
          <Benefit
            icon={<Truck className="h-6 w-6" />}
            title="Entrega Colombia"
            desc="Cobertura nacional y logística confiable"
          />
        </div>
        ...
        <div className="max-w-2xl mx-auto bg-card border rounded-2xl p-8 shadow-card">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <PrivacyNotice>
              Utilizaremos la información suministrada para elaborar y enviar la cotización
              solicitada, conforme a nuestra <LegalLink doc="privacidad" />.
            </PrivacyNotice>
          </form>
        </div>
      </section>
...
function Benefit({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="bg-card border rounded-xl p-6 hover:shadow-card transition-smooth">
      <div className="h-12 w-12 rounded-lg bg-secondary/10 text-secondary flex items-center justify-center mb-3">
        {icon}
      </div>
